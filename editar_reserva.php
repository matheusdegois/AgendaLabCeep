...
// Buscar reserva
$stmt = $pdo->prepare("SELECT * FROM agendamentos WHERE AGE_ID = ?");
$stmt->execute([$id]);
$reserva = $stmt->fetch();
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
        ...
        if (!$is_admin) {
            if ($professor_id != $professores[0]['PRO_ID']) {
                $_SESSION['erro'] = "Você só pode editar reservas do seu próprio cadastro!";
                header("Location: editar_reserva.php?id=" . $id);
                exit;
            }
        }

        // Verificar se já existe reserva para esta combinação (excluindo a atual)
        ...
        $stmt_verifica->execute([$laboratorio_id, $data, $turno_id, $aula_id, $id]);
        ...
        <div class="card border-0 shadow-sm">
            <div class="card-header bg-dark text-white d-flex flex-wrap justify-content-between align-items-center gap-2 py-2">
                <h5 class="card-title mb-0">
                    <i class="fas fa-calendar-edit me-2"></i><?= $titulo ?>
                </h5>
                    ...
                    <div class="row mb-4">
                            ...
                            <select class="form-select form-control-lg" id="laboratorio" name="laboratorio" required>
                                ...
                                <?php foreach ($laboratorios as $lab): ?>
                                    <option value="<?= htmlspecialchars($lab['LAB_ID']) ?>" 
                                        <?= ($lab['LAB_ID'] == $reserva['AGE_LAB_ID'] || (isset($_POST['laboratorio']) && $_POST['laboratorio'] == $lab['LAB_ID'])) ? 'selected' : '' ?>>
                            ...
                            <?php if ($is_admin): ?>
                                ...
                                <select class="form-select form-control-lg" id="professor" name="professor" required>
                                    ...
                                    <?php foreach ($professores as $prof): ?>
                                        <option value="<?= htmlspecialchars($prof['PRO_ID']) ?>" 
                                            <?= ($prof['PRO_ID'] == $reserva['AGE_PRO_ID'] || (isset($_POST['professor']) && $_POST['professor'] == $prof['PRO_ID'])) ? 'selected' : '' ?>>
                            ...
                            <?php else: ?>
                                ...
                                <select class="form-select form-control-lg readonly-combobox" id="professor" name="professor" required disabled>
                                    <?php if (!empty($professores)): ?>
                                        <option value="<?= htmlspecialchars($professores[0]['PRO_ID']) ?>" selected>
                                            <?= htmlspecialchars($professores[0]['PRO_NOME']) ?> <?= htmlspecialchars($professores[0]['PRO_SOBRENOME']) ?>
                                        </option>
                                    <?php else: ?>
                                        <option value="<?= htmlspecialchars($_SESSION['professor_id'] ?? $_SESSION['usuario_id']) ?>" selected>
                                            Meu cadastro
                                        </option>
                                    <?php endif; ?>
                                </select>
                                <input type="hidden" name="professor" value="<?= !empty($professores) ? htmlspecialchars($professores[0]['PRO_ID']) : htmlspecialchars($_SESSION['professor_id'] ?? $_SESSION['usuario_id']) ?>">
                            <?php endif; ?>
                        </div>
                        ...
                        <div class="col-md-4 mb-3">
                            ...
                            <input type="date" class="form-control form-control-lg" id="data" name="data" 
                                   ...
                                   max="<?= $semanaAtual['fim'] ?>"
                                   value="<?= isset($_POST['data']) ? htmlspecialchars($_POST['data']) : htmlspecialchars($reserva['AGE_DATA']) ?>" required>
                        ...
                        <div class="col-md-4 mb-3">
                            ...
                            <select class="form-select form-control-lg" id="turno" name="turno" required>
                                ...
                                <?php foreach ($turnos as $turno): ?>
                                    <option value="<?= htmlspecialchars($turno['TUR_ID']) ?>" 
                                        <?= ($turno['TUR_ID'] == $reserva['AGE_TUR_ID'] || (isset($_POST['turno']) && $_POST['turno'] == $turno['TUR_ID'])) ? 'selected' : '' ?>>
                        ...
                        <div class="col-md-4 mb-3">
                            ...
                            <select class="form-select form-control-lg" id="aula" name="aula" required>
                                ...
                                <?php foreach ($aulas as $aula): ?>
                                    <option value="<?= htmlspecialchars($aula['AUL_ID']) ?>" data-turno="<?= htmlspecialchars($aula['AUL_TUR_ID']) ?>"
                                        <?= ($aula['AUL_ID'] == $reserva['AGE_AUL_ID'] || (isset($_POST['aula']) && $_POST['aula'] == $aula['AUL_ID'])) ? 'selected' : '' ?>>
                                        Aula <?= htmlspecialchars($aula['AUL_AULA_NUMERO']) ?> (<?= htmlspecialchars(substr($aula['AUL_HORARIO_INICIO'], 0, 5)) ?> - <?= htmlspecialchars(substr($aula['AUL_HORARIO_FIM'], 0, 5)) ?>)
                                    </option>
                                <?php endforeach; ?>
                            </select>
                        </div>
