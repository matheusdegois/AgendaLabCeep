...
$where = "LAB_EXCLUIDO = 0";
if (!empty($busca)) {
    $where .= " AND LAB_NOME LIKE :busca";
}

// Total de laboratórios ativos
$sql_count = "SELECT COUNT(*) FROM laboratorios WHERE $where";
...
// Buscar laboratórios com paginação
$sql = "SELECT * FROM laboratorios WHERE $where ORDER BY LAB_NOME LIMIT :limite OFFSET :offset";
...
<div class="container-fluid px-4 py-3">
    <div class="card border-0 shadow-sm">
        <div class="card-header bg-dark text-white d-flex flex-wrap justify-content-between align-items-center gap-2 py-2">
            ...
            <a href="cadastro_laboratorio.php" class="btn btn-success" 
               style="min-height: 44px; display: flex; align-items: center; justify-content: center;">
                <i class="fas fa-plus-circle me-2"></i><span>Novo Laboratório</span>
            </a>
        </div>
        ...
        <div class="card-body">
                    ...
                    <form method="GET" class="d-flex">
                        <div class="input-group">
                            <input type="text" name="busca" class="form-control form-control-lg" 
                                   style="min-height: 44px;" placeholder="Buscar laboratório..." 
            ...
            <p class="text-muted small mb-3">
                <i class="fas fa-database me-1"></i> Exibindo <?= count($laboratorios) ?> de <?= $total ?> laboratório(s) cadastrados.
                <?php if (!empty($busca)): ?>
                    ...
                    <tbody>
                        <?php if (count($laboratorios) > 0): ?>
                            <?php foreach ($laboratorios as $lab): ?>
                                <tr>
                                    <td><?= htmlspecialchars($lab['LAB_NOME']) ?></td>
                                    <td class="text-center">
                                        <div class="d-flex justify-content-center gap-2">
                                            <a href="editar_laboratorio.php?id=<?= $lab['LAB_ID'] ?>" 
                                               class="btn btn-warning" title="Editar" 
                                               style="min-width: 44px; min-height: 44px; display: flex; align-items: center; justify-content: center;">
                                                <i class="fas fa-edit fa-fw"></i>
                                            </a>
                                            <a href="remover_laboratorio.php?id=<?= $lab['LAB_ID'] ?>" 
                                               class="btn btn-danger" title="Remover"
                                               style="min-width: 44px; min-height: 44px; display: flex; align-items: center; justify-content: center;"
                                               onclick="return confirm('Tem certeza que deseja remover o laboratório: <?= htmlspecialchars($lab['LAB_NOME']) ?>?')">
                            ...
                            <tr>
                                <td colspan="2" class="text-center py-3">
                                    <div class="alert alert-info mb-0">
                                        ...
                                        <?= !empty($busca) ? 'Nenhum laboratório encontrado com o termo pesquisado.' : 'Nenhum laboratório cadastrado.' ?>
                                    </div>
                                </td>
                            </tr>
                        <?php endif; ?>
            ...
            <?php if ($total_paginas > 1): ?>
                <nav aria-label="Navegação de páginas">
                    <ul class="pagination pagination-lg justify-content-center mt-4 flex-wrap">
                        ...
                        <?php
                        // Em dispositivos móveis, mostramos menos páginas
